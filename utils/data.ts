export const events = [
	{
		id: "day-1",
		ilustration: {
			src: "/assets/placeholder.png",
			alt: "ilustration",
			blurDataURL:
				"data:image/webp;base64,",
			width: 350,
			height: 200,
			objectFit: "contain",
		},
		day: "01",
		title: "CONFERENCE",
		description:
			"The event is organized and supported by some of the leading projects in the Polkadot and Kusama ecosystems. Day 1 will consist of a conference during which the ecosystem projects will present their experience with 1+years building on Substrate.",
		cta: "/#day-2",
		partners: [
			{
				image: {
					src: "/assets/substrate.png",
					alt: "substrate logo",
					blurDataURL:
						"data:image/webp;base64,",
					width: 50,
					height: 50,
					objectFit: "contain",
				},
				name: "substrate",
			},
			{
				image: {
					src: "/assets/polkadot.png",
					alt: "polkadot logo",
					blurDataURL:
						"data:image/webp;base64,",
					width: 50,
					height: 50,
					objectFit: "contain",
				},
				name: "polkadot",
			},
			{
				image: {
					src: "/assets/kusama.png",
					alt: "substrate logo",
					blurDataURL:
						"data:image/webp;base64,",
					width: 50,
					height: 50,
					objectFit: "contain",
				},
				name: "kusama",
			},
		],
		date: "20 APRIL",
		place: "IN AMSTERDAM/NETHRELANDS",
	},
	{
		id: "day-2",
		ilustration: {
			src: "/assets/placeholder.png",
			alt: "ilustration",
			blurDataURL:
				"data:image/webp;base64,",
			width: 350,
			height: 200,
			objectFit: "contain",
		},
		day: "02",
		title: "WORKSHOP",
		description:
			"During Day 2, a series of workshops will take place aiming to give an accessible introduction into building in the Substrate ecosystem. The workshops will be directed at runtime and frontend development, as well as infrastructure. No prior experience in Substrate is required.",
		cta: "/#footer",
		partners: [
			{
				image: {
					src: "/assets/substrate.png",
					alt: "substrate logo",
					blurDataURL:
						"data:image/webp;base64,",
					width: 50,
					height: 50,
					objectFit: "contain",
				},
				name: "substrate",
			},
			{
				image: {
					src: "/assets/polkadot.png",
					alt: "polkadot logo",
					blurDataURL:
						"data:image/webp;base64,",
					width: 50,
					height: 50,
					objectFit: "contain",
				},
				name: "polkadot",
			},
			{
				image: {
					src: "/assets/kusama.png",
					alt: "substrate logo",
					blurDataURL:
						"data:image/webp;base64,",
					width: 50,
					height: 50,
					objectFit: "contain",
				},
				name: "kusama",
			},
		],
		date: "21 APRIL",
		place: "IN AMSTERDAM/NETHRELANDS",
	},
]
